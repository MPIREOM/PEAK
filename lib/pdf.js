...
export function generatePDF(aiReport, rec, posData, bankTxns, acctSheet, acctIssues, baristaData) {
  ...
  // Escape structured data fields (item names, categories, bank narrations, the
  // sheet name) before interpolating them into the report HTML, so a value that
  // legitimately contains &, <, or > can't break the layout. The AI report body
  // is intentionally rendered as HTML (markdown) and is escaped separately inside
  // mdToHtml where needed, so it is not passed through here.
  ...
  const mdToHtml = (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (emInner) {
        out.push(emInner.length <= 48 ? `<p class="subhead">${emInner}</p>` : `<p class="lead">${emInner}</p>`);
        continue;
      }
  ...
  <div class="card no-break">
    ...
    ${cats.map((c) => barRow(esc(c.name), c.amount, max, "#1a8080", `${f2(c.amount)} OMR · ${((c.amount / currSales) * 100).toFixed(0)}%`)).join("")}
  </div>`;
  }
  ...
  <div class="card no-break">
    ...
    <table>
      ...
      <tbody>${deb.map((t) => `<tr><td class="l">${esc(t.date)}</td><td class="l">${esc(t.desc)}</td><td class="l" style="font-size:10px;color:#7a6e5e">${categorizeExpense(t.raw || t.desc)}</td><td style="color:#c0392b;font-weight:600">${t.amount.toFixed(3)}</td></tr>`).join("")}</tbody>
  ...
  <div class="card no-break">
    ...
    <table>
      ...
      <tbody>
        <tr><td class="l">Beans purchased this month</td><td>${beans.added > 0 ? beans.added + " g" : "—"}</td></tr>
        <tr><td class="l">Beans remaining (month-end)</td><td>${beans.end !== null ? beans.end + " g" : "—"}</td></tr>
        <tr><td class="l">Beginning stock</td><td>${beans.begin !== null ? beans.begin + " g" : "not provided"}</td></tr>
...
<head>
<meta charset="UTF-8"/>
<title>The Peak — ${esc(acctSheet)} Owner Report</title>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Source Sans 3',sans-serif;font-size:13px;color:#1a1a16;background:${PATTERN},linear-gradient(168deg,#faf7f0 0%,#f4ecdd 55%,#efe4d2 100%);background-attachment:fixed;line-height:1.6}
  ...
  /* Section card */
  .card{border:1px solid #d0c8b8;border-radius:8px;padding:18px;margin-bottom:16px;page-break-inside:avoid;background:rgba(255,255,255,.9)}
...
<div class="body">
    ...
    <table>
      ...
      <tbody>${top5
        ...
        <tr style="background:${i < 3 ? "rgba(200,149,42,.05)" : "transparent"}">
          ...
          <td class="l" style="font-weight:${i < 3 ? 700 : 400}">${esc(item.name)}</td>
          <td>${item.qty}</td>
  ...
  <div class="card no-break">
    ...
    <table>
      ...
      <tbody>${baristaData.spoilage.map((r) => `<tr><td class="l" style="color:#c0392b">${esc(r.item)}</td><td style="color:#c0392b;font-weight:700">${r.qty}</td></tr>`).join("")}</tbody>
    </table>
  </div>` : ""}
  ...
  <div class="footer">
    ...
    <div>${esc(acctSheet)} • Generated ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</div>
  </div>
