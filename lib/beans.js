...
export function calcBeans(posData, baristaData) {
  ...
  // When beansConsumedCalc is 0 (no coffee drinks counted from the POS), discPct
  // is null. Guard against it: Math.abs(null) is 0, which would fall through to
  // "ok" and falsely report the waste check as passing. Only 0-vs-0 is genuinely
  // "ok"; a real stock movement with no expected usage is "unknown", not passing.
  const status =
    discrepancy === null
      ? "unknown"
      : discPct === null
  ...
  let hint =
    status === "unknown" && missing.length
      ...
      : null;
  // Stock figures are present but no coffee drinks were counted, so expected
  // usage can't be derived — explain that instead of implying stock is missing.
  if (status === "unknown" && !missing.length && beansConsumedCalc === 0)
    hint =
      "No coffee drinks were found in the POS category totals, so expected bean usage can't be computed for comparison.";
